      ...
      return withDedupe(
        async () => {
          ...
          switch (request.method) {
            ...
            case 'wallet_transferFunds': {
              if (state.accounts.length === 0)
                throw new ox_Provider.DisconnectedError()

              const params = request.params[0] ?? {}
              const requestAddress = params.address
              const requestChainId = params.chainId

              const account = requestAddress
                ? state.accounts.find((account) =>
                    Address.isEqual(account.address, requestAddress),
                  )
                : state.accounts[0]
              if (!account) throw new ox_Provider.UnauthorizedError()
              ...
              const result = await getMode().actions.transferFunds({
                address: account.address,
            ...
            case 'wallet_swapFunds': {
              ...
              const result = await getMode().actions.swapFunds({
                address: account.address,
                chainId: requestChainId,
                internal: {
                  client,
